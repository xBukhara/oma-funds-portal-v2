...
interface FundCheck {
  parsedNavTotal: number;
  sumOfInvestorNavs: number;
  discrepancy: number;
...
export default function StatementUploader({ statements, investors }: Props) {
  ...
  async function handleConfirmSend() {
    ...
    setStatusMsg('✉️ Sending investor emails...');
    ...
    <div className={styles.wrap}>
      <div className={styles.header}>
        ...
        <p className={styles.sub}>
          Upload a monthly PDF — you'll see a full preview of all investor NAV changes
        ...
        {file ? (
          <div className={styles.filePreview}>
            ...
            <div>
              ...
              <p className={styles.fileSize}>{(file.size / 1024).toFixed(1)} KB — ready to parse</p>
            </div>
      ...
      <button className={styles.uploadBtn} onClick={handleParse} disabled={!file || !!statusMsg}>
        {statusMsg || 'Parse Statement & Preview →'}
      </button>
  ...
  if (step === 'preview' && preview) {
    const { parsed, investors: invPreviews, fundCheck } = preview;
    ...
    return (
      <div className={styles.wrap}>
        <div className={styles.header}>
          ...
          <p className={styles.sub}>
            Review all changes before sending. Nothing has been saved or emailed yet.
          </p>
        </div>
        ...
        <div style={{ background: 'rgba(14,22,40,0.8)', border: '1px solid var(--card-border)',
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 17, fontWeight: 700, margin: 0 }}>
            {MONTHS[parsed.latestMonth]} {parsed.year} — Fund Summary
          </h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
            {[
              { label: 'Fund NAV (PDF)', value: fmt(parsed.navTotal), color: 'var(--blue)' },
              { label: 'Monthly Return', value: fmtPct(parsed.latestReturn),
                color: parsed.latestReturn >= 0 ? 'var(--green)' : 'var(--red)' },
              { label: 'Months in Statement', value: `${parsed.monthsProcessed}`, color: 'var(--text)' },
              ...
              <div key={i} style={{ background: 'rgba(7,10,18,0.5)', borderRadius: 12, padding: '14px 16px' }}>
                ...
                <p style={{ fontSize: 20, fontWeight: 800, fontFamily: 'var(--font-display)',
          ...
          {/* Fund balance check */}
          <div style={{
            background: fundCheck.isBalanced ? 'rgba(54,211,153,0.06)' : 'rgba(248,113,113,0.06)',
            border: `1px solid ${fundCheck.isBalanced ? 'rgba(54,211,153,0.2)' : 'rgba(248,113,113,0.3)'}`,
            borderRadius: 12, padding: '12px 16px',
          }}>
            <p style={{ fontSize: 13, margin: 0, color: fundCheck.isBalanced ? 'var(--green)' : 'var(--red)' }}>
              {fundCheck.isBalanced ? '✓' : '⚠'} Fund balance check:{' '}
              Sum of investor NAVs = {fmt(fundCheck.sumOfInvestorNavs)} vs
              PDF total = {fmt(fundCheck.parsedNavTotal)}
              {' '}({fmtPct(fundCheck.discrepancyPct)} discrepancy)
              {!fundCheck.isBalanced && ' — Review before sending'}
            </p>
          </div>
        </div>
        ...
        <div style={{ background: 'rgba(14,22,40,0.8)', border: '1px solid var(--card-border)',
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 700,
            margin: '0 0 16px' }}>Monthly Returns in This Statement</h2>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {parsed.monthlyReturns.map(r => (
              <div key={r.month} style={{
                background: 'rgba(7,10,18,0.5)', borderRadius: 10, padding: '10px 14px',
        ...
        <div style={{ background: 'rgba(14,22,40,0.8)', border: '1px solid var(--card-border)',
          ...
          <div style={{ padding: '20px 24px 0' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 15, fontWeight: 700, margin: 0 }}>
              Investor NAV Changes
            </h2>
            <p style={{ fontSize: 13, color: 'var(--muted)', margin: '4px 0 0' }}>
              These are the values that will be written to each investor's dashboard
            </p>
          </div>
          <div style={{ overflowX: 'auto', marginTop: 16 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 700 }}>
              <thead>
                <tr>
                  {['Investor', 'Current NAV', 'New NAV', 'Change ($)', 'Change (%)', 'Email'].map(h => (
                ...
                {invPreviews.map(inv => (
                  <tr key={inv.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    ...
                    <td style={{ padding: '13px 16px',
                      color: inv.change >= 0 ? 'var(--green)' : 'var(--red)', fontWeight: 600 }}>
                      {inv.change >= 0 ? '+' : ''}{fmt(inv.change)}
                    </td>
                    <td style={{ padding: '13px 16px',
                      color: inv.change_pct >= 0 ? 'var(--green)' : 'var(--red)' }}>
                      {fmtPct(inv.change_pct)}
                    </td>
                    <td style={{ padding: '13px 16px' }}>
                      {inv.has_email
              ...
              <tfoot>
                <tr style={{ background: 'rgba(255,255,255,0.02)' }}>
                  <td style={{ padding: '13px 16px', fontWeight: 700, color: 'var(--muted)',
                    fontSize: 11, textTransform: 'uppercase' }}>Total</td>
      ...
      <div className={styles.wrap}>
        <div style={{ background: 'rgba(54,211,153,0.06)', border: '1px solid rgba(54,211,153,0.2)',
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 20 }}>
            <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(54,211,153,0.12)',
              border: '1px solid rgba(54,211,153,0.3)', display: 'flex', alignItems: 'center',
              justifyContent: 'center', fontSize: 20, color: 'var(--green)', flexShrink: 0 }}>✓</div>
            <div>
              ...
              <p style={{ fontSize: 14, color: 'var(--muted)', margin: 0 }}>
                {MONTHS[sendResult.parsed?.monthsProcessed]} {sendResult.parsed?.year} •
